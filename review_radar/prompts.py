...
SEMANTIC_DEDUP_PROMPT = """你是一个多语言文本分析专家。请识别以下关键词列表和痛点列表中的同义词/同义表达，将它们分组合并。
...
**合并规则：**
- 不同语言的同义词要合并（如 "crash"、"闪退"、"崩溃"、"クラッシュ" 是同一个意思）
- 表达方式不同但含义相同的要合并（如 "加载慢"、"loading slow"、"速度太慢"）
- 痛点中语义重复的要合并（如 "更新后闪退" 和 "升级后崩溃"）
- 每组选择最常见/最清晰的表达作为主词（primary）
- 只合并确实是同义的，不要过度合并
...
{{
    "keyword_groups": [
        {{
            "primary": "闪退",
            "synonyms": ["crash", "崩溃", "クラッシュ"]
        }}
    ],
    "pain_point_groups": [
        {{
            "primary": "更新后频繁闪退",
            "synonyms": ["升级后崩溃", "update后crash"]
        }}
    ]
}}

如果没有需要合并的同义词，返回空数组即可。
"""
...
- **多语言痛点统一**：不同国家的痛点可能用不同语言表达同一个问题（如日语"クラッシュ"、英语"crash"、中文"闪退"），你必须将它们识别为同一痛点，在对比表格中统一翻译为中文，并在括号中标注原始语言表达
- 对比各国家的 Top 痛点差异：
  - 全球共性痛点（多个国家都出现的，合并不同语言的同义表达）
  - 地区特有痛点（仅某国出现的）
